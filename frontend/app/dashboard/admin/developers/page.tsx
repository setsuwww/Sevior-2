...
import { Search, Code2, Plus, ArrowUpDown, Eye, Trash2, Edit, Copy, Mail, Phone } from "lucide-react";
                                        ...
                                        <tr key={dev.ID} className="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end space-x-1">
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-500 hover:text-gray-700 hover:bg-gray-100" onClick={() => copyToClipboard(dev.Email, "Email")} title="Copy Email">
                                                        <Mail className="w-4 h-4" />
                                                    </Button>
                                                    {dev.Phone && (
                                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-gray-500 hover:text-gray-700 hover:bg-gray-100" onClick={() => copyToClipboard(dev.Phone, "Phone")} title="Copy Phone">
                                                            <Phone className="w-4 h-4" />
                                                        </Button>
                                                    )}
                                                    <Link href={`/dashboard/admin/developers/${dev.ID}`}>
                                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-600 hover:text-blue-800 hover:bg-blue-50" title="Detail">
                                                            <Eye className="w-4 h-4" />
                                                        </Button>
                                                    </Link>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-amber-600 hover:text-amber-800 hover:bg-amber-50" onClick={() => alert("Edit feature coming soon!")} title="Edit">
                                                        <Edit className="w-4 h-4" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-red-600 hover:text-red-800 hover:bg-red-50" onClick={() => handleDelete(dev.ID)} title="Delete">
