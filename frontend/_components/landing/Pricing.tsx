            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isFree ? 'bg-white border border-slate-300 shadow-xs rounded-3xl md:rounded-r-none md:border-r-0 z-10 hover:-translate-y-1' : ''}
              ...
              >
                ...
                <div className="mb-8">
                  <h4 className={`text-2xl font-bold mb-2 ${isPro ? "text-white" : "text-slate-900"}`}>
                    {plan.name}
                  </h4>
                  <p className={`text-sm h-10 ${isPro ? "text-teal-100" : "text-slate-500"}`}>
                    {plan.description}
                  </p>
                </div>

                <div className="mb-8 flex items-baseline text-5xl font-extrabold">
                  <span className={isPro ? "text-white" : "text-slate-900"}>{plan.price}</span>
                  <span className={`text-lg font-medium ml-2 ${isPro ? "text-teal-200" : "text-slate-500"}`}>{plan.period}</span>
                </div>
                  ...
                  {plan.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center space-x-3">
                      {feature.included ? (
                        ...
                        <XCircle className={`w-5 h-5 shrink-0 ${isPro ? "text-slate-500" : "text-slate-300"}`} />
                      )}
