      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {services.map((service, index) => (
            <div
              key={index}
              className={`${service.colorHover} bg-white rounded-sm p-8 border border-slate-400 ring ring-offset-4 ring-slate-200 dark:border-slate-800 shadow-xs hover:shadow-sm hover:-translate-y-1 transition-all duration-300 group`}
            >
              ...
              <h4 className="text-xl font-bold text-slate-600 dark:text-white mb-3">
                {service.title}
              </h4>
