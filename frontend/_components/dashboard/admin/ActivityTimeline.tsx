...
export function ActivityTimeline() {
    const getIcon = (type: string) => {
        switch (type) {
            ...
            default: return <div className="w-2 h-2 rounded-full bg-olive-400" />;
        }
    };
    ...
    const getBgColor = (type: string) => {
        switch (type) {
            ...
            default: return "bg-olive-50 border-olive-100";
        }
    };
    ...
    return (
        <div className="bg-white rounded-2xl p-6 border border-olive-200/60 shadow-sm col-span-1 lg:col-span-2 flex flex-col">
            ...
            <div className="relative mt-2 pl-2">
                <div className="absolute left-6 top-2 bottom-0 w-px bg-olive-100"></div>
                ...
                <div className="space-y-6 relative z-10">
                    {MOCK_RECENT_ACTIVITY.map((activity) => (
                        <div key={activity.id} className="flex items-start">
                            ...
                            <div>
                                <p className="text-sm font-medium text-olive-900">{activity.message}</p>
                                <p className="text-xs text-olive-400 mt-1">{activity.time}</p>
                            </div>
                        </div>
                    ))}
                </div>
